...
interface InputProps {
  ...
  select?: boolean;  // whether to render as select dropdown
  options?: SelectOption[];  // options for select dropdown
}
      ...
      <div className="relative">
        ...
        {select ? (
          <select
            ...
            className={`block w-full px-3 py-2.5 bg-white border border-gray-300 rounded-lg text-gray-900 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#465D96]/50 focus:border-[#465D96] transition-all duration-200 ${
              error ? "border-red-500 focus:ring-red-500/50 focus:border-red-500" : ""
            } ${prefixIcon ? "pl-10" : ""} ${disabled ? "bg-gray-50 text-gray-500 cursor-not-allowed" : ""} ${className}`}
          >
            {options.map((option) => (
