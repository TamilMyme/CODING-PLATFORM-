        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-hidden animate-in slide-in-from-bottom duration-300">
            ...
            <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-6 text-white">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-bold">Batch Details</h3>
                  <p className="text-indigo-100 mt-1">View comprehensive batch information</p>
                </div>
                <button
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-4 pb-6 border-b border-gray-200">
                  <div className="relative">
                    <div className="w-20 h-20 bg-gradient-to-br from-indigo-400 to-purple-500 rounded-2xl flex items-center justify-center text-white text-3xl font-bold">
                      {viewingBatch.name?.charAt(0).toUpperCase() || 'B'}
                    </div>
                  </div>
                  <div>
                    <h4 className="text-2xl font-bold text-gray-900">{viewingBatch.name}</h4>
                    <p className="text-gray-600">Batch ID: {viewingBatch._id?.slice(-8) || 'N/A'}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded-lg px-4 py-3">
                    <p className="text-xs text-gray-500 mb-1">Associated Course</p>
                    <p className="text-gray-900 font-medium">{getCourseTitle(viewingBatch.course)}</p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg px-4 py-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-indigo-900">Total Students</span>
                      </div>
                      <p className="text-3xl font-bold text-indigo-600">{viewingBatch.students?.length || 0}</p>
                    </div>
                    <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg px-4 py-4">
                      <div className="flex items-center gap-2 mb-2">
                        <CheckIcon className="w-5 h-5 text-green-600" />
                        <span className="text-sm font-medium text-green-900">Enrollment Status</span>
                      </div>
                      <p className="text-2xl font-bold text-green-600">
                        {viewingBatch.students && viewingBatch.students.length > 0 ? 'Active' : 'No Students'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {viewingBatch.students && viewingBatch.students.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {viewingBatch.students.map((student, index) => {
                        const studentData = typeof student === 'string' 
                          ? students.find(s => s._id === student)
                          : student;
                        return (
                          <div key={index} className="flex items-center gap-3 bg-gray-50 rounded-lg px-4 py-3">
                            ...
                            <div className="flex-1">
                              <p className="text-sm font-medium text-gray-900">{studentData?.name || 'Unknown Student'}</p>
                              <p className="text-xs text-gray-500">{studentData?.email || 'No email'}</p>
                            </div>
                          </div>
                        );
                      })}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-gray-50 rounded-lg px-4 py-3">
                      ...
                      <p className="text-gray-900 font-medium">
                        {new Date(viewingBatch.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                    <div className="bg-gray-50 rounded-lg px-4 py-3">
                      <p className="text-xs text-gray-500 mb-1">Updated Date</p>
                      <p className="text-gray-900 font-medium">
                        {new Date(viewingBatch.updatedAt).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                </div>
